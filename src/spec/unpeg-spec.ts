...
\`\`\``
	...
	it('should properly unpeg with the "me" keyword', async (done: DoneFn) => {
		let database = createDatabase();
		let utilities = createUtilities(1);
		...
		let sentMessage = createMessage('<p><spark-mention data-object-type="person" data-object-id="' + constants.botId + '">' + constants.botName + '</spark-mention> unpeg me because reasons</p>');
		let roomId = 'abc';

		let result = await unpeg.createMessage(sentMessage, roomId);

		expect(result).toEqual({
			markdown: 'It looks like MockSender Name has hidden their pegs too well for me to find them!'
		});
